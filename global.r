...
lea_sel <- lea %>%
  select(
    "county",
    "schoolname",
    starts_with("enroll_allgrades"),
    "ccd_pupil_teacher_ratio",
    "exp_total",
    "rev_total",
    "prof_total",
    "ccd_free_reduced_lunch_eligible",
    "prof_teach_salary",
    "enrollment_lowinc_pct",
    "enrollment_lep",
    "gradrate_4year_total",
    "grads_college_pct",
    "prof_teach_salary",
    "staff_total_ft",
    starts_with("safesch")
...
lea_sel <-
  lea_sel %>% mutate(
    exp_sus = rowSums(select(., starts_with("safesch"))),
...
lea_sel$support_per_student[is.infinite(lea_sel$support_per_student)] <-
  NA

lea_sel$grads_college_pct[lea_sel$county=="Philadelphia"] <- NA
...
census_data <-
  census_data %>% select(-moe) %>% pivot_wider(names_from = variable, values_from = estimate)

census_data <-
  census_data[-501, ] # Remove "school district not defined" row

census_data <-
  census_data %>% mutate(NAME = toupper(
    gsub("School District, Pennsylvania", replacement = "SD", .$NAME)
  )) # Prepare SD names for join
...
unmatched_lea <-
  lea_sel %>% anti_join(census_data, by = c("schoolname" = "NAME"))
unmatched_census <-
  census_data %>% anti_join(lea_sel, by = c("NAME" = "schoolname"))
...
lea_plus <-
  lea_sel %>% left_join(census_data, by = c("schoolname" = "NAME"))
lea_plus <- lea_plus %>% relocate(GEOID, .after = medincome)
lea_plus <-
  lea_plus %>% mutate(support_per_student = round(support_per_student, 2))

# Fix character columns

lea_plus <-
  lea_plus %>% mutate(
    ccd_free_reduced_lunch_eligible = as.numeric(ccd_free_reduced_lunch_eligible),
    ccd_free_reduced_lunch_eligible_pct = ccd_free_reduced_lunch_eligible /
      enroll_allgrades_total * 100,
...
lea_plus <-  lea_plus %>%
  group_by(county) %>%
  mutate_at(vars(contains("enroll_allgrades")), funs(pct_ttl = round(. /
                                                                       enroll_allgrades_total * 100, 2))) %>% mutate_if(is.numeric, round, digits =
                                                                                                                          2)  %>% filter(schoolname != "BRYN ATHYN SD") %>% mutate_if(is.numeric, round, digits =
                                                                                                                                                                                        2)
...
lea_counties <- lea_plus %>%
  ...
  summarise_at(vars(
    -c(
      ...
      "enroll_allgrades_asian",
      "enroll_allgrades_ame",
      "enroll_allgrades_black",
      "enroll_allgrades_hisp",
      "enroll_allgrades_white",
      "enroll_allgrades_tr",
      ends_with("pct_ttl")
    )
  ), roundedMean) %>% mutate_if(is.numeric, round, digits = 2)
...
lea_counties <-
  lea_counties %>% left_join(lea_counties_2, by = "county")
lea_counties <-
  lea_counties %>% left_join(lea_counties_3, by = "county")
lea_counties <-
  lea_counties %>% left_join(lea_counties_4, by = "county") %>% mutate_if(is.numeric, round, digits =
                                                                            2)
...
counties@data <-
  counties@data %>% left_join(lea_counties, by = c("COUNTY_NAM" = "county"))
...
shp <-
  readOGR(
    "data/geo_export_a978505f-e2a6-4942-9816-2fb5f567f17d/",
